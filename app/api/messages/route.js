...
// Your verified ID from the screenshot
...
export async function POST(req) {
  try {
    ...
    if (body.adminAction && !isAdmin) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    ...
    return NextResponse.json({ success: true });
  } catch (e) { return NextResponse.json({ error: "DB Error" }, { status: 500 }); }
}
...
export async function GET(req) {
  ...
  if (server === "staff-room" && !isAdmin) {
    return NextResponse.json([{ text: "🔒 Staff Only", user: "System", system: true }]);
  }
