...
export default function Home() {
  ...
  if (!user) return <div className="bg-[#1e1f22] h-screen text-center p-20">Please login...</div>;
  ...
  return (
    <div className="flex h-screen bg-[#313338] text-white">
      ...
      <div className="w-[72px] bg-[#1e1f22] flex flex-col items-center py-3 gap-2">
        {['GEN', 'STA'].map(s => (
          <div key={s} onClick={() => setActiveServer(s === 'STA' ? 'staff-room' : 'general')}
               className={`w-12 h-12 rounded-full flex items-center justify-center cursor-pointer transition-all ${activeServer.includes(s.toLowerCase()) ? "bg-[#5865f2] rounded-2xl" : "bg-[#313338] hover:rounded-2xl hover:bg-[#5865f2]"}`}>
            {s}
          </div>
        ))}
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="h-12 border-b border-[#232428] flex items-center px-4 font-bold"># {activeServer}</div>
        
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.map((m, i) => (
            <div key={i} className={`flex gap-3 ${m.isAnnounce ? 'bg-[#5865f2]/10 border-l-4 border-[#5865f2] p-2' : ''}`}>
              <img src={m.pfp || "https://api.dicebear.com/7.x/bottts/svg?seed=fallback"} className="w-10 h-10 rounded-full" />
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-bold" style={{ color: m.isAdmin ? "#f1c40f" : "white" }}>{m.user}</span>
                  {m.isAdmin && <span>👑</span>}
                  {m.displayUid && <span className="text-[10px] text-gray-400">ID: {m.displayUid}</span>}
                </div>
        ...
        <div className="p-4 relative">
          ...
          <input 
            ...
            onChange={(e) => {
              ...
              const list = ["/clear", "/announce", "/shrug", "/dice"];
              const match = list.find(c => c.startsWith(e.target.value));
              setSuggestion(match && e.target.value.startsWith("/") ? `Suggest: ${match}` : "");
            }}
            onKeyDown={(e) => e.key === "Enter" && send()}
            className="w-full bg-[#383a40] p-3 rounded-lg outline-none focus:ring-1 focus:ring-[#5865f2]"
